...
def do_put(client, args):
    ...
    if not args.sharelink:
        ...
        q = Queue()
        sleep_q = Queue(maxsize=args.sleeptime)
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            while True:
                if not sleep_q.empty():
                    ...
                    # with tqdm(desc=now_time, bar_format=bar_format) as bar:
                    #     while globle_sleep_time['time'] > 0:
                    #         bar.postfix = [message.format(
                    #             globle_sleep_time['time']
                    #         )]
                    #         time.sleep(1)
                    #         globle_sleep_time['time'] -= 1
                else:
                    task = q.get()
                    ...
                    def get_session_task(task):
                        ...
                        if status == 'good':
                            result = upload_file(
                                upload_url, local_path, remote_path, args.chunk, args.step
                            )
