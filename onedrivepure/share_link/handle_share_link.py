...
stream_file_params = {"@listUrl": "'/personal/{account_name}/Documents'", "View=": ""}
...
def handle_link(link, save_dir, show_json=False):
    if link.startswith("!"):
        link = link[1:]
        is_file = True
    else:
    ...
    dict_data = {"data": data, "sharelink": link}
...
def get_stream_file(s, file_path, tenant_name, account_name):
    url = stream_file_link.format(tenant_name=tenant_name, account_name=account_name)
    ...
    return [{"download_url": download_url, "path": path, "size": size}]


def get_stream_list(
    s, ref_path, tenant_name, account_name, add_params=None, option=464647
):
    ...
    params["@a1"] = params["@a1"].format(account_name=account_name)

    url = stream_link.format(tenant_name=tenant_name, account_name=account_name)
    data = json.dumps(
        {
            "parameters": {
                "__metadata": {"type": "SP.RenderListDataParameters"},
    ...
    for row in list_data["Row"]:
        ...
        if is_dir:
            ...
            result_list.append(
                {"download_url": download_url, "path": path, "size": size}
            )

    next_href = list_data.get("NextHref")
    if next_href:
        params = {
            i.split("=")[0]: i.split("=")[1] for i in next_href.strip("?").split("&")
        }
        result_list += get_stream_list(
            s, ref_path, tenant_name, account_name, add_params=params, option=167943
        )
